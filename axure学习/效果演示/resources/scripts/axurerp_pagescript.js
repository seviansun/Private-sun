...
function IsTrueMouseOut(idNoSpace, e) {
    ...
    while (reltg != null && reltg.nodeName != 'HTML') {
        ...
        if (reltg && reltg.id == idNoSpace) return false;
    }
    return true;
}
...
function ScrollToWidget(id, scrollX, scrollY, easing, duration) {
    ...
    if (!scrollX) targetLeft = document.body.scrollLeft;

    var targetTop = getAbsoluteTop(target);
    if (!scrollY) targetTop = document.body.scrollTop;
    ...
    if (easing == 'none') {
        ...
        $('html,body')
        .animate({ scrollTop: targetTop, scrollLeft: targetLeft}, duration, easing);
    }
}
...
function MoveWidgetBy(id, x, y, easing, duration, animationCompleteCallback) {
    ...
    var widget = $('#' + id);
    if (easing == 'none') {
        var oldTop = Number(widget.css('top').replace("px", ""));
        var oldLeft = Number(widget.css('left').replace("px", ""));
        widget.css('top', oldTop + y);
        widget.css('left', oldLeft + x);
    } else {
        $('#' + id).animate({
            left: '+=' + x,
            top: '+=' + y
...
function GetWidgetText(id) {
    var idQuery = $('#' + id);
    if (idQuery.is('div')) {
        return idQuery.find('div[id$="_rtf"]').text();
...
function ApplyImageStyle(id, event, style) {
    ...
    if (diagramObject.type != 'imageBox' || (style.image && style.image.cssStyle)) {
        ...
        img.removeClass().html("<IMG src='" + style.image.url + "' class='raw_image'>");
    }
}
...
function ResetImagesAndTextJson(id, textid) {
    ...
    if (e && e['_normal']) {
        ...
        if (gv_OriginalImgSrc[id + '_img'] && gv_OriginalImgSrc[id + '_img'] != 'none') {
            img.html("<IMG src='" + gv_OriginalImgSrc[id + '_img'] + "' class='raw_image'>");
        } else {
