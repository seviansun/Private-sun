    ...
    $axure.fn.text = function() {
        if (arguments[0] == undefined) {
            ...
            if(widgetType == TEXT_BOX_TYPE || widgetType == TEXT_AREA_TYPE) { //For non rtf
                ...
                var idRtf = '#' + firstId + '_rtf';

                if ($(idRtf).length == 0){
                    idRtf = '#u' + (parseInt(firstId.substring(1)) + 1) + '_rtf';
                }

                if ($(idRtf).length != 0){
                    var textOut = $(idRtf).text();
...
(function () {
    ...
    var getVariableValue = function(variable) {
        if(_globalVariableValues[variable]) return _globalVariableValues[variable];
        switch (variable) {
...
(function() {
    ...
    var _attachTreeNodeEvent = function(id, eventName, fn) {
        ...
        _attachObjectEvent(id, eventName, fn);
    };

    var _attachObjectEvent = function(id, eventName, fn) {
        ...
        $axure(function(dObj) { return dObj.annotation; }).each(function(dObj) {
            if (dObj.type == 'hyperlink') {
                ...
                for (var i = 0; i < scriptIds.length; i++) {
                    ...
                    var annPos = $('#' + id + 'ann').position();
                    var left = annPos.left - NOTE_SIZE;
                    var top = annPos.top;

                    $('#' + textId).append("<div id='" + id + "Note' class='annnoteimage' style='left:" + left + ";top:" + top + ";'></div>");
...
$(document).ready(function () {
    ...
    var rawPageId = $axure.pageData.page.packageId;
    var formattedPageId = [
    ...
    $(document).bind('keyup', function (e) {
        ...
        if (e.keyCode == '13') {
            var submitButton = $(document.activeElement).attr('axSubmit');
